...
bounded URL-encoded fields and UTF-8 multipart text fields, while non-UTF-8
values and every `filename` or `filename*` part return `400`. Ferrite does not
yet stream request bodies, create upload temp files, propagate client
disconnects to a running action, or claim native upload behavior beyond the
exact tested host platform.

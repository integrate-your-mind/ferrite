...
Ferrite accepts a narrow one-request HTTP/1.1 upstream contract: exact CRLF framing, origin-form targets, one valid `Host`, no request `Transfer-Encoding` or `Expect`, and positive request bodies only for `POST /_ferrite/action` with one decimal `Content-Length`. It rejects ambiguous framing and security-sensitive duplicate headers, malformed authorities, unsupported target/version forms, and already-buffered pipelined suffixes. Every response closes the connection. Production deployments should use the authority-rejecting, buffering, and header-sanitizing nginx template documented in `docs/deployment.md`, not expose Ferrite as a general-purpose public HTTP edge. A reproducible 25-case raw-TLS matrix passes locally against official nginx 1.29.3; hosted proxy/CDN behavior remains unproven.

...
The safest alpha offer today is a controlled internal dashboard, docs portal, or content-heavy app using the documented `ferrite check`, `ferrite build`, and `ferrite serve` path. The remaining market blockers are an immutable production build-to-serve artifact path, removal of request-time bundling and global render serialization, remote GitHub/CI proof, hosted staging proof behind the deployment templates, real npm/native publish proof, full server-action session/replay/auth hardening, and external tracing or audit sink integration.
...
ferrite serve --project examples/basic --host 127.0.0.1 --port 3000 --render-timeout-ms 30000 --request-read-timeout-ms 5000 --max-request-bytes 16384 --max-in-flight-requests 64 --server-action-csrf-token-env FERRITE_ACTION_CSRF --server-action-csrf-cookie-name ferrite_action_csrf --server-action-replay-ttl-ms 300000 --trusted-proxy-public-origin https://app.example.com --trusted-proxy-client-ip-hops 1 --access-log json --action-log json --metrics-path /__ferrite/metrics
...
The `serve` command uses the production HTTP adapter. It scans app routes once, writes generated route types, renders matched requests through the same stream-capable page renderer with configurable render timeout, request-read timeout, request-byte, and in-flight request limits, emits client assets under `/_ferrite/static`, adds conservative `Cache-Control` and `X-Ferrite-Route-Pattern` metadata, returns validated server-payload protocol JSON for `?__ferrite_payload=server`, returns line-delimited shell/chunk frames for `?__ferrite_payload=stream`, accepts same-origin form `POST /_ferrite/action` server-action submissions, requires a valid `Host` header for action POSTs, can require a hidden action CSRF token from `--server-action-csrf-token-env`, can bind that token to a generated SameSite/HttpOnly/Secure cookie with `--server-action-csrf-cookie-name`, can require one-time production action replay nonces with `--server-action-replay-ttl-ms`, rejects browser action POSTs when a present `Origin` or `Referer` host differs from `Host`, can opt into trusted-proxy origin validation with `--trusted-proxy-public-origin`, can opt into a trusted `X-Forwarded-For` client-IP hop policy for logs with `--trusted-proxy-client-ip-hops`, gzip-compresses eligible HTML and payload responses when `Accept-Encoding` allows it, and serves chunked HTTP only when deferred stream chunks exist. Long-running sockets are handled by a reusable worker pool bounded by request-read, request-byte, and in-flight request limits; renderer timeouts return `504 Gateway Timeout`; Rust callers can attach production request observers that receive method, path, status, route pattern, derived client IP, and elapsed duration, plus production action observers that receive action id, route path, route pattern, status, accepted/rejected outcome, derived client IP, and elapsed duration. CLI users can emit those events to stderr with `--access-log plain|json` and `--action-log plain|json`, and can expose in-memory Prometheus text request/action counters with `--metrics-path`; those paths do not log or label headers, bodies, form fields, CSRF tokens, replay nonces, or raw dynamic URL values. Embedders can use the shutdown-aware listener to stop accepting new sockets and drain accepted requests before returning. It does not expose dev manifests, inject the dev reload client, or add `data-ferrite-build-id`.

The worker pool now counts accepted and queued sockets against `--max-in-flight-requests` and returns `503 Service Unavailable` when full. `--render-timeout-ms` applies to renderer and client-bundler subprocesses; deadline failures return generic `504` HTML, and non-timeout subprocess failures return generic `500` HTML while detailed diagnostics stay on server stderr.

...
- A Rust dev server that serves discovered app routes, consumes each successful compiler-owned module graph, and advances its reload build id when app files, graph dependencies, TypeScript configuration, or higher-priority resolver candidates change, appear, or disappear.

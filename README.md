...
- A Rust `ferrite-protocol` crate that owns render-packet, render-stream, client-reference, and server-payload markers, versioning, packet shapes, and validation.
...
- Dev and production HTTP adapters can return validated `server-payload` protocol JSON for matched routes with `?__ferrite_payload=server`.
- A production HTTP adapter that renders app routes on demand through the stream-capable path, serves generated client assets, emits conservative cache/deployment metadata, returns server-payload responses on request, and omits dev reload state.
...
- Browser helpers that fetch, validate, and apply full server-payload responses through the existing DOM update path.
- A payload-backed browser navigator that intercepts safe same-origin links, updates route roots/history from server payloads, and falls back to normal navigation for failed or bypassed requests.

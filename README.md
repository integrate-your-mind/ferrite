...
- A payload-backed browser navigator that prefetches safe same-origin targets, intercepts safe links, updates route roots/history from server payloads, restores marked back/forward entries, and falls back to normal navigation for failed or bypassed requests.

...
The `serve` command uses the production HTTP adapter. It scans app routes once, writes generated route types, renders matched requests through the same stream-capable page renderer, emits client assets under `/_ferrite/static`, adds conservative `Cache-Control` and `X-Ferrite-Route-Pattern` metadata, returns validated server-payload protocol JSON for `?__ferrite_payload=server`, returns line-delimited shell/chunk frames for `?__ferrite_payload=stream`, and serves chunked HTTP only when deferred stream chunks exist. It does not expose dev manifests, inject the dev reload client, or add `data-ferrite-build-id`.

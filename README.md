...
The safest alpha offer today is a controlled internal dashboard, docs portal, or content-heavy app using the documented `ferrite check`, `ferrite build`, and `ferrite serve` path. The remaining market blockers are remote GitHub/CI proof, hosted staging proof behind the deployment templates, real npm/native publish proof, full server-action session/replay/auth hardening, and external tracing or audit sink integration.

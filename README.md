...
Ferrite accepts a narrow one-request HTTP/1.1 upstream contract: exact CRLF framing, origin-form targets, one valid `Host`, no request `Transfer-Encoding` or `Expect`, and positive request bodies only for `POST /_ferrite/action` with one decimal `Content-Length`. It rejects ambiguous framing and security-sensitive duplicate headers, malformed authorities, unsupported target/version forms, and already-buffered pipelined suffixes. Every response closes the connection. Production deployments should use the authority-rejecting, buffering, and header-sanitizing nginx template documented in `docs/deployment.md`, not expose Ferrite as a general-purpose public HTTP edge. `pnpm test:nginx:stack` builds the production image, starts it with nginx 1.29.3 pinned by immutable index digest on an isolated Docker network, generates ephemeral local TLS material, and runs the 25-case raw-TLS matrix plus fail-closed controls and verified cleanup. Hosted proxy/CDN behavior remains unproven.

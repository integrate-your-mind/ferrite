...
- A Rust `ferrite-protocol` crate that owns render-packet/render-stream markers, versioning, packet shapes, and validation.
...
- Client reference manifests, browser chunks, and automatic island hydration for `"use client"` components imported by otherwise server-rendered routes.
...
- A production HTTP adapter that renders app routes on demand through the stream-capable path, serves generated client assets, emits conservative cache/deployment metadata, and omits dev reload state.
...
Routes are server-first by default. A page or layout that starts with a `"use client"` directive opts the whole matched route into browser hydration and route-specific client bundling. When a server route imports a separate `"use client"` module, the production manifest records a `clientReferences` entry for that module/export and emits a browser chunk for it while keeping the route itself scriptless. The server renderer automatically proxies that imported client component into an island container with `data-ferrite-client-reference`, JSON `data-ferrite-client-props`, and server fallback HTML. The referenced chunk hydrates that subtree without hydrating the whole route. This is an island contract, not full React Server Components yet.

## Current Boundaries

Ferrite now has a small browser DOM runtime, hydration proof, keyed element reconciliation, core state/effect/ref/memo hooks, layout-effect-before-passive timing, priority transition scheduling, cooperative yielding before transition DOM commits, error boundary fallback routing, server `Suspense` fallback shells for async child components, route-level loading/error file conventions, Rust dev/build/serve route handling, chunked dev and production HTTP output for streamed route chunks, conservative production cache-control and route-pattern metadata headers, TSX page-module execution for server output, root layout composition, app-owned document shells, page/layout metadata composition with Open Graph/icons/alternates, directive-gated whole-route browser bundling with CSS/source maps/assets, client reference manifests plus emitted and loaded browser chunks for automatically proxied imported-client islands, generated static output for known static, dynamic, and catch-all routes, duplicate static output detection, dev and production static asset serving, a native Rust render protocol crate, a Rust-generated TypeScript protocol mirror, a loadable native Node SSR addon, versioned compact render packets, and versioned render-stream packets for the JS-to-Rust SSR boundary. It does not yet implement a published WASM package, resumable Fiber-style rendering, or React Server Components. The JavaScript runtime package is a typed facade over the intended API while the Rust core owns validated tree construction, SSR primitives, route analysis, legacy serialized VNode compatibility, render-packet/render-stream decoding, dev route serving, production output, and CLI orchestration.

...
The safest alpha offer today is a controlled internal dashboard, docs portal, or content-heavy app using the documented `ferrite check`, `ferrite build`, and artifact-backed `ferrite serve` path. The remaining market blockers are remote GitHub/CI proof, hosted staging and rollback proof behind the deployment templates, real npm/native publish proof, public CLI distribution and starter onboarding, sustained production load/soak and hosted process-recovery evidence, full server-action session/replay/auth hardening, and external tracing or audit sink integration.
...
Production admission, artifact validation, subprocess and response-write deadlines, public error redaction, source-independent dynamic route/action execution, overlapping request execution, controlled saturation recovery, and artifact-runner failure recovery are locally tested. Remaining runtime proof gaps are sustained capacity/soak testing, concurrent slow-reader load, host-process supervisor recovery, hosted proxy/TLS behavior, and horizontally coordinated replay state.

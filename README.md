...
Ferrite accepts a narrow one-request HTTP/1.1 upstream contract: exact CRLF framing, origin-form targets, one valid `Host`, no request `Transfer-Encoding` or `Expect`, and positive request bodies only for `POST /_ferrite/action` with one decimal `Content-Length`. It rejects ambiguous framing and security-sensitive duplicate headers, malformed authorities, unsupported target/version forms, and already-buffered pipelined suffixes. Every response closes the connection. Production deployments should use the buffering and header-sanitizing nginx template documented in `docs/deployment.md`, not expose Ferrite as a general-purpose public HTTP edge.

## Current Boundaries

Production admission, strict HTTP/1.1 request framing, artifact validation, subprocess and response-write deadlines, public error redaction, source-independent dynamic route/action execution, overlapping request execution, bounded mixed-load soak, concurrent slow-reader deadline recovery, controlled saturation recovery, and artifact-runner failure recovery are locally tested. Remaining runtime proof gaps are hosted capacity benchmarking, long-duration soak, host-process supervisor recovery, hosted proxy/TLS behavior, and horizontally coordinated replay state.

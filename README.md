...
- Dev and production HTTP adapters can accept same-origin form `POST /_ferrite/action` submissions for explicit route-scoped server action ids, parse urlencoded or small text multipart fields, invoke the matched route through the existing page-renderer boundary, and return validated `server-action-response` JSON. The DOM runtime also exposes server-action form enhancement that submits those forms with `FormData` and validates the response packet; generated route and client-reference browser entrypoints install that enhancement idempotently when browser JavaScript is present. Build/dev manifests now record explicit `createServerAction()` form references discovered during route render. This is still explicit form transport; automatic `"use server"` discovery, deployment-stable inferred action ids, dedicated server-only-route bootstrap assets, and client event invocation remain future work.

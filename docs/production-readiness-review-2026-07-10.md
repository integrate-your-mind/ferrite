...
- A self-contained harness builds that production image, runs it behind official nginx 1.29.3 pinned by immutable index digest on an isolated Docker network, executes 25 raw-TLS framing/proxy cases, proves five fail-closed controls, and verifies handled-path cleanup.

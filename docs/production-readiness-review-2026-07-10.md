...
The general Verify workflow and candidate-image nginx harness exist, but every relevant hosted Actions attempt has failed before job allocation. A credential-free GitHub API probe at 2026-07-18T11:17:36Z returned HTTP 404 for the repository, PR #4, reviews, comments, and checks, so current authenticated state cannot be inferred from the cached refs. Local base/head merge-tree construction is clean and matches the documentation checkpoint tree, but that does not substitute for GitHub mergeability, branch protection, or checks. There is no hosted deployment. Local exact-commit proof cannot substitute for real hosted CI, registry, or staging evidence.

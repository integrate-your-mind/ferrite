...
- `docs/milestone-001-proof.md` through `docs/milestone-075-proof.md`
- `package.json`
...
- [ ] Runtime DOM coverage now includes Chromium proof for generated server-action form enhancement, payload prefetch/navigation, stream-frame navigation, stream-mode popstate restoration, and malformed programmatic payload rejection, but hydration mismatch paths, malformed click fallback behavior, failed popstate fallback behavior, and many focus/pointer/history edge cases still rely on deterministic `happy-dom` coverage.
...
- [ ] Browser runtime proof now includes Chromium coverage for generated server-action form enhancement, payload prefetch consumption, browser URL/title updates, stream-frame navigation, stream-mode popstate restoration, and malformed programmatic payload rejection without DOM/history mutation. Malformed click fallback behavior, failed popstate fallback behavior, hydration mismatch paths, and many focus/pointer/history edge cases still mostly rely on Node plus `happy-dom`.
...
- [ ] Add real browser tests for broader hydration behavior, payload navigation, streaming, popstate restoration, and prefetching. Payload prefetch/navigation, stream-frame navigation, valid stream-mode popstate restoration, and malformed programmatic payload rejection now have Chromium proof; hydration mismatch paths, malformed click fallback behavior, failed popstate fallback behavior, and broader focus/pointer/history edges still need browser coverage.

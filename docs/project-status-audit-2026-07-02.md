...
- [x] Add opt-in application-session-cookie and exact-route binding for process-local replay nonces without storing raw session values.
- [ ] Complete server-action CSRF/session/replay hardening: authenticated session integration, session-bound CSRF token issuance/rotation, and multi-process replay coordination.

...
A missing, empty, malformed, or ambiguous configured session cookie causes route rendering to omit the replay nonce and action POSTs to return the same generic replay rejection used for a wrong route, wrong session, expired nonce, or reused nonce. Binding mismatches do not consume the legitimate nonce. Rotating the app session cookie makes nonces issued to the old session unusable by the new session; rotating the configured CSRF secret makes older rendered forms fail CSRF validation without consuming their nonce. Entries remain process-local and expire or are evicted at the configured bounds. Use one sticky Ferrite process for the complete GET/action flow; multiple processes or regions require an external atomic consume store that this release does not provide. Omitting `--server-action-session-cookie-name` preserves the prior unbound replay behavior.

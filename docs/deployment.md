...
This command requires Node.js, OpenSSL, and a reachable Docker daemon. It reports the Git commit/tree and whether the worktree is clean, pulls official nginx 1.29.3 by immutable multi-platform index digest, builds the checked-out production container with source-revision labels, creates an isolated bridge network, and runs the built artifact behind an ephemeral TLS listener. It first proves that nginx rejects missing certificate material, validates the generated config with `nginx -t`, then executes the 25-case matrix. Additional negative controls prove default certificate validation, mandatory upstream log evidence, the response-size cap, a stalled-TLS deadline, and synthetic smuggling-canary detection. Handled success, failure, `SIGINT`, and `SIGTERM` paths remove the uniquely named containers, network, candidate image tag, log follower, and scratch directory and verify their absence. An uncatchable process kill, host loss, or Docker daemon loss can still interrupt cleanup; resources use the `ferrite-proof-` or `nginx-proof-` name prefixes so an operator can inspect ownership before removing leftovers.

The harness runs the production image and nginx as separate containers on the same private bridge network. It does not rely on Docker host networking or a platform-specific host-gateway alias. The successful local certificate is intentionally self-signed; the harness disables verification only for the successful matrix and separately proves that the secure default rejects it. Run this command from a clean exact commit for release evidence. The harness prints the candidate image id, source labels, nginx digest and architecture, and the Ferrite production build id emitted by the image build.
...
- First-pass container, systemd, and nginx templates exist with local static verification; the self-contained Docker harness builds the current artifact-only production image and runs a real 25-case TLS framing matrix plus fail-closed controls against official nginx 1.29.3 pinned by index digest. No official container image, Helm chart, managed platform adapter, or hosted staging proof exists yet.
...
- Local nginx framing parity is reproducibly proven by the candidate-image harness for official nginx 1.29.3, including client-side chunked actions, authority rejection, mandatory upstream evidence, and handled-path cleanup. Linux amd64 CI execution, the exact hosted proxy image, real TLS chain, CDN behavior, restart, and rollback remain proof gaps until hosted jobs actually run.

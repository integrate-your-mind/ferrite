...
- `--server-action-session-cookie-name`: optional app-owned session cookie used to bind each replay nonce to the issuing session fingerprint and exact route; it requires `--server-action-replay-ttl-ms`, must differ from `--server-action-csrf-cookie-name`, and leaves pages renderable without a valid session cookie while withholding a replay nonce so their action forms fail closed
...
Current production hardening is incomplete. Ferrite can require one configured hidden server-action CSRF token, bind that token to a SameSite/HttpOnly/Secure double-submit cookie, and optionally bind process-local replay nonces to an app-owned session cookie plus exact route. The session binding stores only a domain-separated SHA-256 fingerprint in the bounded replay store. Before handling real authenticated mutations it still needs:

- first-class authenticated session validation and session-bound CSRF token issuance/rotation
...
A missing, empty, malformed, or ambiguous configured session cookie causes route rendering to omit the replay nonce and action POSTs to return the same generic replay rejection used for a wrong route, wrong session, expired nonce, or reused nonce. The session cookie must therefore exist on the request that renders the form; if login creates it in that response, redirect or reload before rendering an actionable form. Binding mismatches and malformed action packets do not consume the legitimate nonce. A failed route render discards its unreachable nonce. Once Ferrite accepts a valid action packet, however, the nonce remains spent even if action lookup, invocation, or rendering later returns `404`, `500`, or `504`, because the mutation outcome may be unknown. Render a fresh form before retrying.

Binding is deliberately scoped to the exact route and session fingerprint, not to a specific action ID. Every action must still perform its own authentication, authorization, and input validation; a route-level nonce is not an authorization capability. Rotating the app session cookie value makes nonces issued to the old value unusable by the new value. Rotating only an application signing key while preserving identical cookie bytes is not observable to Ferrite and does not invalidate those nonces; rotate the cookie value or restart the Ferrite process as part of that change. Rotating the configured CSRF secret makes older rendered forms fail CSRF validation without consuming their nonce.

Entries remain process-local and expire or are evicted at the configured bounds. Use one sticky Ferrite process for the complete GET/action flow; multiple processes or regions require an external atomic consume store that this release does not provide. Process restart clears all outstanding nonces. Omitting `--server-action-session-cookie-name` preserves the prior unbound replay behavior. Rust embedders that need deterministic bound-form rendering can call `ProductionProject::handle_get_with_headers`; `handle_get` and CLI `serve --once` intentionally supply no cookies.

Rotate the referenced CSRF secret as part of the deployment process. The configured token must be cookie-safe when cookie binding is enabled. If the public TLS origin differs from the upstream Ferrite bind origin, set `--trusted-proxy-public-origin` and configure the proxy to own and sanitize the forwarded proto/host headers. If access logs need public client IPs behind the proxy, set `--trusted-proxy-client-ip-hops` to the exact number of trusted proxy hops and make the edge proxy overwrite `X-Forwarded-For`.

...
- Local nginx framing parity is proven for the exact recorded source, resolved platform image, and candidate build produced by the harness, including client-side chunked actions, authority rejection, mandatory upstream evidence, and handled-path cleanup. Mutable candidate base tags, native Windows execution, Linux amd64 CI execution, the exact hosted proxy image, real TLS chain, CDN behavior, restart, and rollback remain proof gaps until those paths run.

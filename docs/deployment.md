...
- First-pass container, systemd, and nginx templates exist with local static verification; the self-contained Docker harness builds a clean exact Git archive into the artifact-only production image and runs 37 raw HTTP/1 TLS cases, four negotiated HTTP/2 cases, proxy-level no-upstream framing checks, and fail-closed controls against official nginx 1.29.3 selected by immutable index digest. No official container image, Helm chart, managed platform adapter, or hosted staging proof exists yet.
...
- Local nginx framing parity is proven for the immutable exact-commit build context, recorded Git tree, resolved platform image, and candidate build produced by the harness, including client-side chunked actions, proxy-level no-upstream rejection for five ambiguous framing probes, authority rejection, mandatory Ferrite log evidence, and handled-path cleanup. Mutable candidate base tags, native Windows execution, Linux amd64 CI execution, the exact hosted proxy image, real TLS chain, CDN behavior, restart, and rollback remain proof gaps until those paths run.

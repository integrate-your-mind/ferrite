...
const featureRows = [
  ...
  ["Available", "Server-side rendering", "Request-time rendering supports layouts, metadata, escaping, streaming, and artifact-backed serving."],
  ["Available", "Client-side rendering", "The browser runtime mounts and hydrates typed TSX routes and explicit client islands."],
  ["Available", "Static site generation", "Production builds can prerender routes into deterministic HTML and immutable browser assets."],
  ["Partial", "Server actions", "CSRF, replay, proxy, audit, and public-error controls exist; auth is incomplete."],
  ["Experimental", "Cloudflare edge deployment", "This site serves verified prerendered Ferrite output; request-time Ferrite SSR does not run in a Worker."],
...
export default function HomePage() {
  return <>
    <section className="hero"><div className="wrap hero-grid"><div>
      ...
      <p>Ferrite is an experimental app runtime with server rendering, browser rendering and hydration, and static prerendering behind one explicit artifact contract.</p>
    ...
    <section className="section"><div className="wrap"><p className="kicker">04 / Architecture</p><div className="dark-panel"><h2>Source → compiler → artifact → serve</h2><p>Ferrite compiles this application through the <code>@ferrite/runtime</code> app-directory contract. The same validated artifact can drive request-time server rendering, browser hydration, or prerendered static delivery. This Cloudflare deployment serves the prerendered mode; it does not run Ferrite's native HTTP server inside a Worker.</p><pre>{`app/page.tsx       →   ferrite-cli check/build\napp/layout.tsx     →   SSR + metadata + document\n"use client"       →   browser mount/hydration bundle\nprerendered routes →   static HTML + immutable assets\n.ferrite/build     →   artifact-only native serve`}</pre></div></div></section>
